...
dplyr::inner_join(geo, cuisine, by="PLACEID") %>% select(RCUISINE, PRICE, STATES) %>% ggplot(aes(x=RCUISINE, y=PRICE)) + geom_point() + geom_jitter(alpha=1, width=.1) + facet_wrap(~STATES)

dplyr::left_join(cuisine, ratings, by="PLACEID") %>% select(RCUISINE, RATING, FOOD_RATING, SERVICE_RATING) %>% ggplot(aes(x=RCUISINE, y=RATING)) + geom_point() + geom_jitter(alpha=1, width=.05)
